...
import com.infinity.infoway.bmu.activity.EmployeeFillAttendanceActivity;
...
public class LeaveLectureAdapter extends BaseAdapter {


    private Context ctx;
    Lectures benefits = new Lectures();
...
//
    ...
    public LeaveLectureAdapter(Lectures benefits, Context ctx) {
        this.ctx = ctx;
        this.benefits = benefits;
        inflater = LayoutInflater.from(this.ctx);
        selected = new ArrayList<>();
        if (benefits != null && benefits.getTable() != null && benefits.getTable().size() > 0) {
            for (int i = 0; i < benefits.getTable().size(); i++) {
                selected.add(benefits.getTable().get(i).getDL_LEC_NO() + "");
            }
        }
...
//        CustomTextView txt_status_counter;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        //   holder.txtsrno.setText("  " + benefits.getTable().get(position).getSrno() + "");
//        holder.txtpreviouslectstatus.setText("  " + benefits.getTable().get(position).getPre_att_status() + "");
        ...
        holder.txtsrno.setText(benefits.getTable().get(position).getDL_LEC_NO() + "");
...
//        holder.cbcheck.setChecked(selected.contains(benefits.getTable().get(position).getDL_ID() + ""));
        holder.cbcheck.setChecked(selected.contains(benefits.getTable().get(position).getDL_LEC_NO() + ""));
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (buttonView.isShown()) {
                    if (!buttonView.isChecked()) {
                        selected.remove(benefits.getTable().get(position).getDL_LEC_NO() + "");
                        //selected_name.remove( "1");
                    } else if (buttonView.isChecked()) {
                        if (!selected.contains(benefits.getTable().get(position).getDL_LEC_NO() + "")) {
                            selected.add(benefits.getTable().get(position).getDL_LEC_NO() + "");
